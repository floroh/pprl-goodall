...
def add_plot_size_sidebar():
    # st.session_state[PLOT_RESIZE] = st.sidebar.checkbox(label="Plot resize",
    #                                                     key="plotresize")
...
def plot_gini(
    ...
    y_limit: float = None,
    width: int = 600,
    title="Gini Index by Attribute-level Bloom Filter",
) -> Figure:
    ...
    if "CLASSIFICATION" in prj.phases:
        ...
        if state_exists_and_equals("knowledge_mode", "Evaluation"):
            ...
            if report.type == "TEXT":
                ...
                quality_history.update_xaxes(
                    title_standoff=20, ticklabelposition="inside", ticklabelstandoff=10
                )
