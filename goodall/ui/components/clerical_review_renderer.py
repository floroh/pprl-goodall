...
from goodall.ui.components.api.lu_api_streamlit import (
    ...
    get_record_pairs_cached,
    get_records_by_unique_id,
)
...
class ClericalReviewRenderer:
    ...
    def render_clerical_review(self) -> bool:
        ...
        get_record_pairs_cached.clear()  # Clear cached pairs from previous iteration
        ...
        mask_index = get_state_or_default("selected_mask_index", 2)
        mask_name = st.selectbox("Masking", MASKS.keys(), index=mask_index)
        mask_index = list(MASKS.keys()).index(mask_name)
        sts["selected_mask_index"] = mask_index
        pair_displays = []
        for pair in pairs:
        ...
        if get_state_or_default("view_mode", "Simple") == "Expert":
            with st.expander("Full"):
                st.json(pairs)
                st.dataframe(
                    get_record_pairs_as_dataframe_cached(layer.project_id, ["new"])
                )
    ...
    def render_labeling_quality_summary(
        self, pair_displays: list[PairDisplay], labelings: list[int]
    ):
        correct_counter = 0
        ...
        if completed > 0:
            for idx, label in enumerate(labelings):
                ...
                if gt is not None and gt == sel_string:
                    correct_counter += 1
            st.info(
                f"{correct_counter}/{completed} "
                f"({int(100 * correct_counter / completed)}%) are correct"
            )
    ...
    def get_records(self, pairs):
        if "records" in sts:
            ...
            records = get_records_by_unique_id(rids)
            sts["records"] = records
        return records
    ...
    def get_new_pairs(layer: Layer):
        if "pairs" in sts:
            ...
            pairs = get_record_pairs_cached(layer.project_id, ["new"])
            sts["pairs"] = pairs
        return pairs

    def render_pair_displays(
        self,
        ...
        for pair_idx, pair_display in enumerate(pair_displays):
            ...
            with col_sel:
                ...
                labelings[pair_idx] = (
                    options.index(new_selection) if new_selection is not None else None
                )
                sts["manual_pair_labelings"] = labelings
            with col_feedback:
                if show_correctness and new_selection is not None:
                    ...
                    sel_string = (
                        "TRUE_MATCH"
                        if new_selection == options[1]
                        else "TRUE_NON_MATCH"
                    )

                    st.markdown(
                        """<div height='100px'></div>""", unsafe_allow_html=True
                    )
                    if gt == sel_string:
                        st.success("Correct")
                    else:
    ...
    def show_autofill_labels_options(
        self, pair_displays: list[PairDisplay], labelings: list[int]
    ):
        with st.expander("Autolabel configuration"):
            autofill_mode = st.segmented_control(
                "Autolabel mode",
        ...
        if btn_auto_fill:
            ...
            def label_based_on_error_rate(value, row_number):
                if value is None:
                    ...
                    return (
                        inverse_gt_label_display
                        if random() < sel_error_rate
                        else gt_label_display
                    )
                return value  # Keep the original value
            ...
            if autofill_mode == "Match rate":
                ...
                labelings = [
                    label_based_on_error_rate(label, idx)
                    for idx, label in enumerate(labelings)
                ]
            else:
