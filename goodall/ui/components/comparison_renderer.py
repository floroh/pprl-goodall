...
from goodall.ui.streamlit_utils import sts
from goodall.protocol.clerical_review.pair_display import (
    format_bitset_as_bitstring,
    PairDisplay,
)
...
def subsubheader(subsubheader_text: str, style: str = "Default"):
    ...
    st.markdown(
        f"""<div style='{css_style}'>{subsubheader_text}</div>""",
...
def calculate_similarity(str1, str2, method="jaro-winkler"):
    if method == "jaro-winkler":
        ...
        return 1 - (
            jellyfish.levenshtein_distance(str1, str2) / max(len(str1), len(str2))
        )
...
STS_DEMO_PAIR = "mlal_demo_example_pair"

def update_pair_example():
    selection = sts["pair_selection"]

    if selection == "Match":
        sts[STS_DEMO_PAIR] = get_example_true_match_pair()
    else:
        sts[STS_DEMO_PAIR] = get_example_true_non_match_pair()
...
def render_edit_form(input_pair: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for row_idx, record_data in enumerate(input_pair):
        record_data = {
            key: value for key, value in record_data.items() if "FRQ" not in key
        }
        ...
        for i, (attribute, value) in enumerate(record_data.items()):
            if "FRQ" in attribute:
                continue
            updated_value = columns[i].text_input(
                label=attribute,
                value=value,
                key=f"{attribute}_{value}_{row_idx}",
                label_visibility="visible" if row_idx == 0 else "collapsed",
            )
            updated_record[attribute] = updated_value

        updated_input_pair.append(updated_record)

    if STS_DEMO_PAIR not in st.session_state:
        st.session_state[STS_DEMO_PAIR] = input_pair

    if st.button("Update"):
        st.session_state[STS_DEMO_PAIR] = updated_input_pair

    return st.session_state[STS_DEMO_PAIR]


def render_multi_layer_comparison_examples() -> List[Dict[str, str]]:
    st.subheader("Record comparison approaches")
    options = ["Match", "Non-match"]

    if STS_DEMO_PAIR not in st.session_state:
        st.session_state[STS_DEMO_PAIR] = (
            get_example_true_match_pair()
        )

    if "pair_selection" not in st.session_state:
        st.session_state["pair_selection"] = options[0]

    st.segmented_control(
        "Select record pair example",
        options,
        key="pair_selection",
        on_change=update_pair_example,
    )

    with st.expander("Edit records"):
        input_pair = render_edit_form(sts[STS_DEMO_PAIR])
    render_multi_layer_comparison_example(input_pair)

def render_multi_layer_comparison_example(
        input_pair: List[Dict[str, str]] | None = None,
        gt_label: str | None = None,
        show_encodings: bool = True,
) -> List[Dict[str, str]]:
    records = []
    for idx, record_data in enumerate(input_pair):
        ...
        record = lu.RecordDto(
            id=lu.RecordIdDto(unique=record_id), attributes=attributes
        )
    ...
    pair = lu.RecordPairDto(
        id0=records[0].id,
        id1=records[1].id,
        tags=[lu.Tag(tag="Groundtruth-Label", string_value=gt_label)],
    ...
    pair_display_modes = st.segmented_control(
    ...
    if options[1] in pair_display_modes:
        rbf_record_dtos = [
            do_api.encode(record, "DBSLeipzig/RBF/FNLNDOBCITY")
            for record in do_record_dtos
        ]
        ...
        pair = lu.RecordPairDto(
            ...
            id1=records[1].id,
            tags=[lu.Tag(tag="Groundtruth-Label", string_value=gt_label)],
            attribute_similarities=calculate_attribute_similarities(
                rbf_record_dtos, method="binary-dice"
            ),
        )
        if show_encodings:
    ...
    if options[2] in pair_display_modes:
        abf_record_dtos = [
            do_api.encode(record, "DBSLeipzig/FBF/FNLNDOBCITY")
            for record in do_record_dtos
        ]
        ...
        pair = lu.RecordPairDto(
            ...
            tags=[lu.Tag(tag="Groundtruth-Label", string_value=gt_label)],
            attribute_similarities=calculate_attribute_similarities(
                abf_record_dtos, method="binary-dice"
            ),
        )
        # st.json(pair)
    ...
    if options[3] in pair_display_modes:
        ...
        subsubheader(
            "Mask equal, very dissimilar attributes, as well as equal characters"
        )
