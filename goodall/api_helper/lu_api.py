...
def create_encoded_dataset(
    plaintext_dataset_id: int, dataset_name: str = ""
) -> lu.DatasetDto:
    return dataset_controller.add_dataset_description(
        lu.DatasetDto.from_dict(
            {"plaintextDatasetId": plaintext_dataset_id, "datasetName": dataset_name}
        )
    )
...
def run_linkage_result_analysis(
    plain_records: List[RecordDto], request: lu.AnalysisRequestDto
):
...
def get_record_pairs_as_dataframe(project_id: str, properties: list) -> pd.DataFrame:
    return get_as_dataframe(get_record_pairs(project_id, properties))


def get_as_dataframe(pairs: list[lu.RecordPairDto]) -> pd.DataFrame:
...
def get_records(dataset_id: int, limit: int = -1) -> list[RecordDto]:
    logger.info(f"Getting records for dataset {dataset_id} with limit {limit}")
    if limit >= 0:
...
def update_config(config: lu.MatchingDto):
    config_controller.update2(config)
...
def delete_project(project_id: str, delete_parents: bool = False):
    ...
    #     get_matcher_endpoint() + 'project/' + project_id + '?deleteParents=' + str(
    #         delete_parents))
    # return response
